...
export function Projects() {
  ...
  return (
    <div className="my-36 w-full flex flex-col items-center">
      ...
      <div className="mt-14 grid grid-cols-[20rem_20rem_20rem] grid-rows-[13rem_13rem_13rem] gap-3">
        {projects.map((project) => (
          <button key={project.id} id={String(project.id)} disabled={project.title === '' ? true : false} onClick={() => handleBannerClick(project)} className="transition-all disabled:outline-0 disabled:cursor-not-allowed hover:outline hover:outline-my-blue rounded-lg flex flex-col h-full items-center cursor-pointer">
            <div 
              style={ project.img ? {backgroundImage: `url(${project.img})`} : {backgroundColor: 'black'}} 
              className="bg-cover bg-center w-[20rem] h-[13rem] rounded-lg rounded-b-none"
            />
            <div className="bg-black py-2 w-full rounded-lg rounded-t-none">
                <strong className="text-white">{project.title !== '' ? project.title : `#${project.id} Coming Soon...`}</strong>
            </div>
          </button>
        ))}
        <Modal
